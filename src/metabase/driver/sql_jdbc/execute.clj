...
(defn- prepared-statement*
  ^PreparedStatement [driver conn sql params canceled-chan & {:keys [fetch-size]}]
...
(defn- run-fn-with-result-set
  ...
  (let [db (qp.store/database)
    ...
    (with-open [conn (connection-with-timezone driver db (qp.timezone/report-timezone-id-if-supported))
                stmt (doto (prepared-statement* driver conn sql params (context/canceled-chan context) :fetch-size fs)
                       (.setMaxRows max-rows))
